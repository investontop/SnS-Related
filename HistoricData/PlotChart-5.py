# PlotChart
# PlotChart-4.py + change in plot_data_hsec to group the df_buy and df_sell
#                + plot the qty for scatter buy and sold dots
...
def plot_data_kite(platform, df_price, df_trade, stock_name):
    ...
    df_remaining, longTerm_qty, shortTerm_qty = fn_baseLongTerm_calc(df_buy_group, df_sell_group)
    ...
    plt.title(f'Stock Price Over Time for {stockInfo['shortName'] or stock_name}')
    plt.xlabel('Date')
    plt.ylabel('Price')
    plt.grid(True, linestyle='--', alpha=0.6)
    plt.legend()
...
def fn_baseLongTerm_calc(df_buy_group, df_sell_group):
